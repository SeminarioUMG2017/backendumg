...
@Service
public class MailerCatedratico {
	...
	public void executeMail(String idCatedratico , String pass) throws IOException{
	
	 final Locale locale = null;
	 final Context ctx = new Context(locale.US);
	 final Context ctxno = new Context(locale.US);
	 
	 Catedratico catedratico = catedraticoService.findOne(Integer.parseInt(idCatedratico));
	 
	
	 String to = catedratico.getCorreo();
    ...
    try {
     	        ...
     	        Qr	qr = qrservice.findOne(idCatedratico);
     	        
     	        String qrpath = qr.getNoCarnet(); 	   
   

     	       	ctxno.setVariable("name", catedratico.getNombres() );
     	    	ctxno.setVariable("nocarnet", idCatedratico);
     	    	ctxno.setVariable("pass", pass);
     	     	ctxno.setVariable("user", idCatedratico);
     	    	
     	    	System.out.println("Real  " +servletContext.getRealPath("/"));
     	    	System.out.println("Context "+servletContext.getContextPath());
     	    	ctxno.setVariable("qr","http://34.233.183.228:8080/seminario/codigosqr/"+idCatedratico+".png");
     	    	ctxno.setVariable("logo","http://34.233.183.228:8080/seminario/img/logo.jpg"); 
     	    	
     	    
     	    	final String htmlContentno = this.templateEngine.process("emailcatedra", ctxno);   	
